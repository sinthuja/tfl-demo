...
function drawRealtimeChart(data) {
    jQuery("#noChart").html("");
    if (chart == null) {
        chart = new vizg(createDatatable(data), gadgetConfig.chartConfig);
...
function createDatatable(data) {
    ...
    for(var i =0; i < gadgetConfig.columns.length; i++) {
        ...
        if(gadgetConfig.columns[i]["type"] === "int" || gadgetConfig.columns[i]["type"] === "float" || gadgetConfig.columns[i]["type"] === "double") {
            type = "linear";
        } else if (gadgetConfig.columns[i]["type"] == "time") {
            type = "time";
        } else {
            type = "ordinal";
        }
...
function convertData(data) {
    for (var i = 0; i < data.length; i++) {
        for (var x = 0; x < data[i].length; x++) {
            if (gadgetConfig.columns[x]["type"] === "int") {
                data[i][x] = parseInt(data[i][x]);
            } else if (gadgetConfig.columns[x]["type"] === "float" || gadgetConfig.columns[x]["type"] === "double") {
